...
new p5((p5) => {
    ...
    p5.draw = () => {
        ...
        for(let corpo of mundo.bodies) {
            ...
            for(let i=0;i<corpo.forma.normals.length;i++) {
                let vertice = corpo.forma.vertices[i].copia;
                vertice.mult(corpo.forma.u);
                vertice.adic(corpo.posicao);
                let normal = corpo.forma.normals[i].copia;
                normal.mult(corpo.forma.u);
                normal.adic(corpo.posicao);
                p5.stroke("red");
                p5.line(normal.x, normal.y, vertice.x, vertice.y);
            }
        }

        for(let i of mundo.contacts) {
